...
### bayes priors
"""
The prior expresses the values of expected alerts or sequence-id of alert.
There are two types of priors, subjective and objective.  
	Objective priors allow the data to influence the posterior the most.  Objective priors contain important characteristics that reflect objectivity.  A uniform prior over all features may assign too large of probability to non-informative points.  A normal random variable with large variance (small precision) may be better in situations where values naturally tend toward zero.  Or an exponential distribution (with fat tail) in strict positive or negative cases.
	
	Subjective priors can be based on practicioner opinion, calculated posterior probability, or domain knowledge.  It allows one to add more probability mass to certain areas of the prior.

If calculated posterior probabilities appear inaccurate, it implies the priors need to be updated, and do not contain all need information.

Bayesian methods have a prior distribution, with hyperparameters _alpha_, while empirical frequentist methods do not have the notion of a prior.  Empirical Bayes combines the two, by using frequency methods to select _alpha_ , and then proceeding with Bayes.

Empiricial bayes in semi-objective, where the model is selected subjetively, and the parameters are determined by the data objectively.

Priors: Gamma, Wishart, beta

Gamma is r.v. over positive reals.  It is a generalization of the exponential r.v. : Exp(_beta) ~ Gamma(1,_beta).  The additional parameter allows probability density function to have more flexibility.  Subjective priors can be expressed more accurately.  Density function for Gamma(_alpha,_beta) if f(x|_alpha,_beta)=_beta^(_alpha)*x^(_alpha-1)/_gamma(_alpha)

Wishart is random matrices distribution over all positive semi-definite.  Covariance matrices are positive-definite, so Wishart is appropriate prior.

Beta, random variable over [0,1] with density function :  f_x(x|_alpha,_beta) = x^(_alpha-1)(1-x)^(_beta-1)/_beta(_alpha,_beta)
parameters (_alpha=1, _beta=1) is a uniiform distribution.  Beta distribution is generalization of uniform distribution.

"a beta prior with binomial observations creates a beta posterior"
given a unknown probability p, a beta(_alpha, _beta) is assigned prior to p.  And observed data is generated by a binomial process, X ~ Binomial(N,p), with p still unknown.  Then the posterior is again a beta distribution: p|X ~ Beta(_alpha+X, _beta+N-X)
...
### bayes bandits
"""
  scales well, approximately-optimal solution

  online algorithm, begins in ignorant state, as it acquires data it learns which bandit is the best.
