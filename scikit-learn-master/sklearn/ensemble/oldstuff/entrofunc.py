...
#doctest module
#run from commandLine as python module.py
...
This function describes the entropy over the count.
The weight assigned to each feature over the number of classifiers is stored in global WHT.
Intervals are 1/window-size(number of data-points).  The count number is the number of WHT in each interval.
The entropy is calculated over the count-number of the intervals.

The interval number influences the entropy count.  If the intervals are too few (ie 1/50 vs 1/1000), then diversity of states is reduced; entropy reduces to zero.
TODO entCntBayesBin()
...
def entCnt( *args ):
        ...
        c=Counter(); wht = np.copy(*args); arln = 1.0/4150#arln=1.0/wht.shape[0]
        #print 'len wht ', len(*args), wht.shape[0]
        ##_dict key=unique-wht : values=frequency
        # sort by unique, ascending
        for e in wht:
                c[e]+=1
        ...
        ##_count number of val in interval=arln
        frc=[] ; tvr=0
        for i in srt:
                if i < arln:
                        tvr += c[i]
